"""
Meme Generator Web — A meme generator web app where users can describe a meme they want in text, optionally upload an image as a template/reference, and generate a new meme. Needs a text input field, an image upload button, and a Generate Meme button. Dark mode UI, simple and clean design, optimized for mobile.
Built by Sheept 🐑💤 | Type: review | Seed: 2eb0417bac5c
"""
...
app = FastAPI(title="Meme Generator Web", description="A meme generator web app where users can describe a meme they want in text, optionally upload an image as a template/reference, and generate a new meme. Needs a text input field, an image upload button, and a Generate Meme button. Dark mode UI, simple and clean design, optimized for mobile.")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])
DB = "/tmp/meme_generator_web_2eb041.db"
...
def init():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY, name TEXT, description TEXT, category TEXT, avg_rating REAL DEFAULT 0, review_count INTEGER DEFAULT 0, created_at TEXT);
        ...
        CREATE TABLE IF NOT EXISTS ratings (id TEXT PRIMARY KEY, user_id TEXT, item_id TEXT, score INTEGER, created_at TEXT);
...
def list_items(limit: int = 50, offset: int = 0):
    with db() as c: c.execute("SELECT * FROM items ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)); return [dict(r) for r in c.fetchall()]
@app.post("/items")
def create_items(r: ItemsReq, auth: Optional[str] = Header(None)):
    get_user(auth); rid = uid(); d = r.dict()
    cols, vals = ", ".join(["id"] + list(d.keys()) + ["created_at"]), [rid] + list(d.values()) + [now()]
    with db() as c: c.execute(f"INSERT INTO items ({cols}) VALUES ({','.join(['?']*len(vals))})", vals)
    return {"id": rid}
@app.get("/items/{id}")
def get_items(id: str):
    with db() as c: c.execute("SELECT * FROM items WHERE id=?", (id,)); row = c.fetchone()
    if not row: raise HTTPException(404, "Not found")
    return dict(row)
@app.delete("/items/{id}")
def delete_items(id: str, auth: Optional[str] = Header(None)):
    get_user(auth)
    with db() as c: c.execute("DELETE FROM items WHERE id=?", (id,))
...
<head>
  ...
  <title>Meme Generator Web</title>
  <meta name="description" content="A meme generator web app where users can describe a meme they want in text, optionally upload an image as a template/reference, and generate a new meme. Needs a text input field, an image upload button, and a Generate Meme button. Dark mode UI, simple and clean design, optimized for mobile.">
  ...
  <style>
    ...
    .gradient-text {
      background: linear-gradient(135deg, #fc970e, #d97706);
...
<body class="antialiased" style="background: #ffffff; color: #1c1917;">
  <!-- Navigation -->
  <nav class="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl border-b border-gray-200/60" style="background: rgba(255,255,255,0.8);">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <a href="#" class="flex items-center gap-2 text-gray-900 font-bold text-lg">
          <span>🐑</span> <span>Meme Generator Web</span>
        </a>
        <div class="hidden md:flex items-center gap-8">
          <a href="#browse" class="text-gray-900/70 hover:text-gray-600 transition-colors text-sm font-medium">Browse</a>
            <a href="#item_detail" class="text-gray-900/70 hover:text-gray-600 transition-colors text-sm font-medium">Item Detail</a>
            <a href="#write_review" class="text-gray-900/70 hover:text-gray-600 transition-colors text-sm font-medium">Write Review</a>
            <a href="#top_rated" class="text-gray-900/70 hover:text-gray-600 transition-colors text-sm font-medium">Top Rated</a>
      ...
      <div id="mobile-menu" class="hidden md:hidden pb-4 space-y-2">
        <a href="#browse" class="block py-2 text-gray-900/70 hover:text-gray-600 text-sm font-medium">Browse</a><a href="#item_detail" class="block py-2 text-gray-900/70 hover:text-gray-600 text-sm font-medium">Item Detail</a><a href="#write_review" class="block py-2 text-gray-900/70 hover:text-gray-600 text-sm font-medium">Write Review</a><a href="#top_rated" class="block py-2 text-gray-900/70 hover:text-gray-600 text-sm font-medium">Top Rated</a>
      </div>
    </div>
  </nav>
  <div class="h-16"></div>
  ...
  <section class="hero-gradient min-h-[90vh] flex items-center relative overflow-hidden">
    <div class="absolute inset-0 overflow-hidden">
      <div class="absolute -top-40 -right-40 w-80 h-80 rounded-full opacity-20" style="background: #fc970e; filter: blur(100px);"></div>
      <div class="absolute -bottom-40 -left-40 w-96 h-96 rounded-full opacity-10" style="background: #d97706; filter: blur(120px);"></div>
    </div>
    <div class="max-w-6xl mx-auto px-6 lg:px-8 py-20 relative z-10">
      <div class="max-w-3xl">
        <div class="reveal">
          <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium mb-8" style="background: #fc970e15; color: #fc970e;">
            <span class="w-2 h-2 rounded-full" style="background: #fc970e;"></span>
            Now Live
          </div>
        </div>
        ...
        <p class="reveal reveal-delay-1 text-xl sm:text-2xl text-gray-600 leading-relaxed max-w-2xl mb-10">
          A meme generator web app where users can describe a meme they want in text, optionally upload an image as a template/reference, and generate a new meme. Needs a text input field, an image upload button, and a Generate Meme button. Dark mode UI, simple and clean design, optimized for mobile.
        </p>
        <div class="reveal reveal-delay-2 flex flex-col sm:flex-row gap-4">
          ...
          <a href="#features" class="inline-flex items-center justify-center px-8 py-4 rounded-xl text-lg font-semibold text-gray-900/80 hover:text-gray-900 transition-colors" style="border: 1px solid #fde68a;">
            Learn More
          </a>
        </div>
      </div>
  ...
  <!-- Features -->
  <section id="features" class="py-24 lg:py-32" style="background: #fefce8;">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div class="reveal reveal-delay-1 group p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-3xl mb-4">⭐</div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Rating System</h3>
            <p class="text-gray-600 text-sm leading-relaxed">Rate and review with a detailed scoring system everyone can trust.</p>
          </div>
          <div class="reveal reveal-delay-2 group p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-3xl mb-4">⭐</div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Trusted Reviews</h3>
            <p class="text-gray-600 text-sm leading-relaxed">Verified reviews build trust and help buyers make confident decisions.</p>
          </div>
          <div class="reveal reveal-delay-3 group p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-3xl mb-4">📊</div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Score Aggregation</h3>
            <p class="text-gray-600 text-sm leading-relaxed">See aggregated ratings at a glance with smart trend analysis.</p>
          </div>
          <div class="reveal reveal-delay-4 group p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-3xl mb-4">🏷️</div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Categories</h3>
            <p class="text-gray-600 text-sm leading-relaxed">Browse organized categories to quickly find what you're looking for.</p>
          </div>
          <div class="reveal reveal-delay-5 group p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-3xl mb-4">🛡️</div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Moderation</h3>
            <p class="text-gray-600 text-sm leading-relaxed">Community-driven moderation keeps content authentic and trustworthy.</p>
          </div>
      </div>
    </div>
  </section>
  <!-- How It Works -->
  <section id="how-it-works" class="py-24 lg:py-32" style="background: #ffffff;">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
      <div class="text-center mb-16">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div class="reveal reveal-delay-1 flex flex-col items-center text-center">
          <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold mb-5" style="background: #fc970e20; color: #fc970e;">
            1
          </div>
          <h3 class="text-lg font-semibold text-gray-900 mb-2">Find an Item</h3>
          <p class="text-gray-600 text-sm leading-relaxed max-w-xs">Browse categories or search for something specific to review.</p>
        </div>
        <div class="reveal reveal-delay-2 flex flex-col items-center text-center">
          ...
          <p class="text-gray-600 text-sm leading-relaxed max-w-xs">See what others think with detailed ratings and honest feedback.</p>
        </div>
        <div class="reveal reveal-delay-3 flex flex-col items-center text-center">
          <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold mb-5" style="background: #fc970e20; color: #fc970e;">
            3
          </div>
          <h3 class="text-lg font-semibold text-gray-900 mb-2">Write Your Review</h3>
          <p class="text-gray-600 text-sm leading-relaxed max-w-xs">Share your experience and help the community make better choices.</p>
        </div>
        <div class="reveal reveal-delay-4 flex flex-col items-center text-center">
          <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold mb-5" style="background: #fc970e20; color: #fc970e;">
            4
          </div>
          <h3 class="text-lg font-semibold text-gray-900 mb-2">Build Trust</h3>
          <p class="text-gray-600 text-sm leading-relaxed max-w-xs">Your reviews help create a trustworthy, transparent community.</p>
        </div>
      </div>
    </div>
  </section>
  <!-- CTA -->
  <section class="py-24 lg:py-32 relative overflow-hidden" style="background: #1c1917;">
    <div class="absolute inset-0 overflow-hidden">
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full opacity-10" style="background: #fc970e; filter: blur(150px);"></div>
    </div>
    <div class="max-w-4xl mx-auto px-6 lg:px-8 text-center relative z-10">
      ...
      <p class="reveal reveal-delay-1 text-lg text-gray-400 mb-10 max-w-2xl mx-auto">
        Join Meme Generator Web today and experience the difference. No credit card required.
      </p>
  ...
  <section id="app-dashboard" class="hidden py-12 min-h-screen" style="background: #ffffff;">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
      <!-- Welcome -->
      <div class="mb-8">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          ...
          <div class="p-5 rounded-2xl transition-all hover:shadow-md" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-2xl mb-2">⭐</div>
            <div class="text-gray-600 text-xs font-medium uppercase tracking-wider mb-1">Reviews Written</div>
            <div class="text-gray-900 text-2xl font-bold" id="stat-reviews">--</div>
          </div>
          <div class="p-5 rounded-2xl transition-all hover:shadow-md" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-2xl mb-2">📊</div>
            <div class="text-gray-600 text-xs font-medium uppercase tracking-wider mb-1">Avg Rating</div>
            <div class="text-gray-900 text-2xl font-bold" id="stat-avgrating">--</div>
          </div>
          <div class="p-5 rounded-2xl transition-all hover:shadow-md" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-2xl mb-2">🏷️</div>
            <div class="text-gray-600 text-xs font-medium uppercase tracking-wider mb-1">Categories</div>
            <div class="text-gray-900 text-2xl font-bold" id="stat-cats">--</div>
          </div>
          <div class="p-5 rounded-2xl transition-all hover:shadow-md" style="background: #ffffff; border: 1px solid #fde68a;">
            <div class="text-2xl mb-2">👀</div>
            <div class="text-gray-600 text-xs font-medium uppercase tracking-wider mb-1">Items Reviewed</div>
            <div class="text-gray-900 text-2xl font-bold" id="stat-items">--</div>
          </div>
      </div>
      <!-- Content -->
      ...
      <div class="mb-10">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-bold text-gray-900">⭐ Latest Reviews</h2>
        ...
        <div id="reviews-grid" class="space-y-3">
  ...
  <!-- Footer -->
  <footer class="py-12" style="background: #1c1917; border-top: 1px solid rgba(255,255,255,0.05);">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
  ...
  <div id="auth-modal" class="modal-overlay" onclick="if(event.target===this)closeAuth()">
    <div class="w-full max-w-md mx-4 p-8 rounded-2xl shadow-2xl" style="background: #ffffff; border: 1px solid #fde68a;">
      <div class="text-center mb-8">
        <div class="text-4xl mb-3">🐑</div>
        <h2 class="text-2xl font-bold text-gray-900" id="auth-title">Welcome Back</h2>
        <p class="text-sm mt-1" style="color: #a8a29e;" id="auth-subtitle">Sign in to your account</p>
      </div>
      <div class="space-y-4">
        <div>
          ...
          <input id="auth-user" type="text" placeholder="Enter your username"
            ...
            style="background: #fffbeb; border: 1px solid #fde68a; color: #1c1917;">
        </div>
        <div>
      ...
      <div class="mt-6 text-center">
        <span class="text-sm" style="color: #a8a29e;" id="auth-switch-text">Don't have an account?</span>
        <button onclick="toggleAuthMode()" class="text-sm font-semibold ml-1" style="color: #fc970e;" id="auth-switch-btn">Sign Up</button>
      </div>
    </div>
  </div>
  <!-- Create Modal -->
  <div id="create-modal" class="modal-overlay" onclick="if(event.target===this)closeCreateModal()">
    <div class="w-full max-w-md mx-4 p-8 rounded-2xl shadow-2xl" style="background: #ffffff; border: 1px solid #fde68a;">
      <h2 class="text-xl font-bold text-gray-900 mb-6">Write Review</h2>
      <div class="space-y-4">
        
          <div>
            <label class="block text-sm font-medium text-gray-900 mb-1.5">Item</label>
            <input id="create-item_id" type="text" placeholder="Item"
              class="w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-cta/30"
              style="background: #fffbeb; border: 1px solid #fde68a; color: #1c1917;">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-900 mb-1.5">Rating (1-5)</label>
            <input id="create-rating" type="number" placeholder="Rating (1-5)"
              class="w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-cta/30"
              style="background: #fffbeb; border: 1px solid #fde68a; color: #1c1917;">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-900 mb-1.5">Your Review</label>
            <input id="create-comment" type="text" placeholder="Your Review"
              class="w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-cta/30"
              style="background: #fffbeb; border: 1px solid #fde68a; color: #1c1917;">
          </div>
...
<script>
    ...
    try {
      ...
      if (el) {
        el.innerHTML = items.length
          ? items.map(r => `<div class="p-4 rounded-xl hover:shadow-md transition-all" style="background: ${cardBg}; border: 1px solid ${cardBorder};"><div class="flex items-center gap-2 mb-2"><span style="color: ${accent};">${'⭐'.repeat(r.rating||0)}</span></div><p class="text-sm">${r.comment||'No comment'}</p></div>`).join('')
  ...
  async function submitCreate() {
    try {
      const body = { item_id: v('create-item_id'), rating: parseInt(v('create-rating'))||5, comment: v('create-comment')||'' };
